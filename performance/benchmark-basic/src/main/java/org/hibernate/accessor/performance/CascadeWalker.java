 ...
 * beans. Every read goes through {@link ValueReader#get} at the two loops below, so a
...
public final class CascadeWalker {
	...
	public record TypePlan(ValueReader<?>[] scalars, ValueReader<?>[] cascades) {
	}
	 ...
	 * {@link BookSchema}.
	 */
	public static CascadeWalker forStrategy(AccessorFactory factory, CascadeAccess access)
		...
		for ( Class<?> type : BookSchema.ENTITIES ) {
			...
			List<ValueReader<?>> scalars = new ArrayList<>();
			List<ValueReader<?>> cascades = new ArrayList<>();
			...
			plans.put( type, new TypePlan(
					scalars.toArray( new ValueReader<?>[0] ),
					cascades.toArray( new ValueReader<?>[0] ) ) );
		}
	...
	private long visit(Object bean, long acc) {
		...
		for ( ValueReader<?> reader : plan.scalars() ) {
			Object value = reader.get( bean );
			if ( value != null ) {
				acc += value.hashCode();
			}
		}
		for ( ValueReader<?> reader : plan.cascades() ) {
